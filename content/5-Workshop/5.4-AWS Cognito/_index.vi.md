...
- Kết nối với **các nhà cung cấp đăng nhập xã hội (Social IdP)**:
  ...
  - **Facebook** → cho người dùng đăng nhập bằng tài khoản Facebook

Sau khi đăng nhập thành công (bằng email, Google hoặc Facebook), Cognito trả về **token**, và các token này được dùng để bảo vệ API, Lambda và các tài nguyên phía backend.
...
3. **Tạo app client**
   ...
   - Bật **Cognito User Pool** và **social identity providers (Google, Facebook)** trong phần allowed identity providers.
...
## 5.4.3 Bật đăng nhập bằng Google (Gmail) và Facebook

Để hỗ trợ đăng nhập xã hội, chúng tôi thêm **Google** và **Facebook** làm identity provider trong Cognito.
...
1. Vào **Meta for Developers (Facebook Developer)**, tạo một **App** mới và bật **Facebook Login** cho Web.
2. Cấu hình **Valid OAuth Redirect URIs** trỏ về URL callback của Cognito.
3. Lấy **App ID** và **App Secret** từ Facebook.
4. Trong **Cognito → User pool → Identity providers → Facebook**:
   - Dán App ID và App Secret.
   - Map các thuộc tính như email, name của Facebook sang các thuộc tính chuẩn của Cognito.

Sau khi hoàn tất, người dùng có thể chọn **"Sign in with Facebook"** để đăng nhập bằng tài khoản Facebook.
...
- Với **Gmail / Facebook** (đăng nhập xã hội):
  - Gọi `Auth.federatedSignIn({ provider: 'Google' })` hoặc `Auth.federatedSignIn({ provider: 'Facebook' })`.
  - Amplify sẽ:
    1. Chuyển hướng người dùng sang trang đăng nhập Google/Facebook.
    2. Người dùng xác thực thành công.
    3. Google/Facebook trả về Cognito.
    4. Cognito trả về token và chuyển hướng lại frontend của English Journey.

Ví dụ code React (đơn giản):
...
---
**Trên giao diện trang đăng nhập, chúng tôi hiển thị ba lựa chọn chính:**
...
- Mở rộng sau này với:
    ...
    - Chính sách mật khẩu mạnh hơn

**Trong phạm vi workshop này, chúng tôi tập trung vào:**

- Đăng nhập bằng email & mật khẩu

- Đăng nhập xã hội qua Gmail và Facebook
...
- Cho phép người dùng đăng nhập bằng email, Google (Gmail) hoặc Facebook.
